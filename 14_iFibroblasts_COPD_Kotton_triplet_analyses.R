...
# Load COPD clusters, i.e., Clusters 0, 4, and 5
# cluster0.file <- "volcano_fig4_fibroblast_cluster_0_20200716.csv"
# cluster4.file <- "volcano_fig4_fibroblast_cluster_4_20200716.csv"
# cluster5.file <- "volcano_fig4_fibroblast_cluster_5_20200716.csv"
DEGs.iFibr.COPD0 <- read.csv(paste0(table.dir, iFibr.dir, 
                                    "volcano_fig4_fibroblast_cluster_0_20200716.csv"))
colnames(DEGs.iFibr.COPD0)
dim(DEGs.iFibr.COPD0)
range(DEGs.iFibr.COPD0$logfc)
range(DEGs.iFibr.COPD0$adj_pval)

DEGs.iFibr.COPD4 <- read.csv(paste0(table.dir, iFibr.dir, 
                                    "volcano_fig4_fibroblast_cluster_4_20200716.csv"))
colnames(DEGs.iFibr.COPD4)
dim(DEGs.iFibr.COPD4)
range(DEGs.iFibr.COPD4$logfc)
range(DEGs.iFibr.COPD4$adj_pval)

DEGs.iFibr.COPD5 <- read.csv(paste0(table.dir, iFibr.dir, 
                                    "volcano_fig4_fibroblast_cluster_5_20200716.csv"))
colnames(DEGs.iFibr.COPD5)
dim(DEGs.iFibr.COPD5)
range(DEGs.iFibr.COPD5$logfc)
range(DEGs.iFibr.COPD5$adj_pval)

qs::qsave(list(cluster0 = DEGs.iFibr.COPD0, 
               cluster4 = DEGs.iFibr.COPD4, 
               cluster5 = DEGs.iFibr.COPD5), 
          paste0(R.dir, "DEGs_iFibr_COPD_clusters_045.qsave"))
DEGs.iFibr.list <- qs::qread(paste0(R.dir, "DEGs_bimod_Fibr_COPD_public.qsave"))
...
DEGs.Fibr.COPD <- FindMarkers(obj, ident.1 = "COPD", group.by = "Disease", 
                                                    subset.ident = "Fibroblast", 
                              test.use = "bimod")
# “bimod” : Likelihood-ratio test for single cell feature expression, (McDavid et al., Bioinformatics, 2013)
# McDavid A, Finak G, Chattopadyay PK, Dominguez M, Lamoreaux L, Ma SS, Roederer M, Gottardo R. Data exploration, quality control and testing in single-cell qPCR-based gene expression experiments. Bioinformatics. 2013 Feb 15;29(4):461-7. doi: 10.1093/bioinformatics/bts714. Epub 2012 Dec 24. PMID: 23267174; PMCID: PMC3570210.
qs::qsave(DEGs.Fibr.COPD, paste0(R.dir, "DEGs_bimod_Fibr_COPD_public.qsave"))
stop ("Finished DEG prediction via bimodal distribution.\n")
DEGs.Fibr.COPD <- qs::qread(paste0(R.dir, "DEGs_bimod_Fibr_COPD_public.qsave"))
dim(DEGs.Fibr.COPD)


# Merge the DEGs of iFibr (clusters 0, 4, and 5)
intersect(DEGs.iFibr.COPD4$gene, DEGs.iFibr.COPD5$gene)
DEGs.iFibr.COPD <- rbind(DEGs.iFibr.COPD0, DEGs.iFibr.COPD4, DEGs.iFibr.COPD5)
unique(DEGs.iFibr.COPD$cluster_name)
nrow(DEGs.iFibr.COPD) == nrow(DEGs.iFibr.COPD0) + nrow(DEGs.iFibr.COPD4) + nrow(DEGs.iFibr.COPD5)
DEGs.iFibr.COPD <- DEGs.iFibr.COPD[DEGs.iFibr.COPD$cluster_name == "copd",]
dim(DEGs.iFibr.COPD)
range(DEGs.iFibr.COPD$adj_pval)
range(DEGs.iFibr.COPD$volcano_adjp)
range(DEGs.iFibr.COPD$logfc)
range(DEGs.iFibr.COPD$volcano_logfc)
length(unique(DEGs.iFibr.COPD$gene))
...
range(DEGs.Fibr.COPD.filtered$avg_log2FC)
range(DEGs.Fibr.COPD.filtered$p_val_adj)


# Rank the DEGs
DEGs.iFibr.COPD.filtered <- DEGs.iFibr.COPD.filtered[order(DEGs.iFibr.COPD.filtered$pval),]
DEGs.iFibr.COPD.filtered <- DEGs.iFibr.COPD.filtered[!duplicated(DEGs.iFibr.COPD.filtered$gene),]
...
# DEGs.iFibr.COPD.sig <- head(DEGs.iFibr.COPD.sig, n = 860)
# DEGs.Fibr.COPD.sig <- head(DEGs.Fibr.COPD.sig, n = 860)
DEGs.overlap <- intersect(DEGs.iFibr.COPD.sig$gene, rownames(DEGs.Fibr.COPD.sig))
length(DEGs.overlap)
...
iFibr.enriched <- run_GO_and_KEGG(genes.ll = DEGs.iFibr.COPD.sig$gene, org = "human")
