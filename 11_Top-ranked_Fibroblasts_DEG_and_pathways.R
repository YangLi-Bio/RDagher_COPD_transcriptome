...
n.AT2.DEGs <- 150
dir.create(paste0(table.dir, "Fibroblasts"))
Fibro.dir <- paste0(table.dir, "Fibroblasts/")


############################################################################
#                                                                          #
#   Analyses of Fibroblasts :                                              #
#   1. Compare the top-150 DEGs between Fibroblasts v.s. iFibroblasts      #
#   2. GO and KEGG enrichment analyses for Fibroblasts and iFibroblasts,   #
#      respectively                                                        #
...
# Local parameters
iFibro.DEGFile <- "COPD_healthy_DEGs_iAT2.csv"
Fibro.DEGFile <- "COPD_healthy_DEGs.csv"
...
AT2.DEGs <- read.csv(paste0(Fibro.dir, AT2.DEGFile))
dim(AT2.DEGs)
colnames(AT2.DEGs)
...
write.csv(iAT2.top.COPD.DEGs[which(iAT2.top.COPD.DEGs$gene %in% AT2.DEGs.overlapped),], 
          paste0(Fibro.dir, "iAT2_DEGs_overlapped.csv"), 
          quote = F, row.names = F)
write.csv(AT2.DEGs.sorted[which(AT2.DEGs.sorted$gene %in% AT2.DEGs.overlapped),], 
          paste0(Fibro.dir, "AT2_DEGs_overlapped.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.padj.MF, paste0(Fibro.dir, "iAT2_padj_MF.csv"), 
          quote = F, row.names = F)

AT2.padj.MF <- cbind(Overlap = AT2.sig.MF$Term %in% AT2.padj.overlapMF, 
                      AT2.sig.MF)
head(AT2.padj.MF[, 1 : 5])
write.csv(AT2.padj.MF, paste0(Fibro.dir, "AT2_padj_MF.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.pval.MF, paste0(Fibro.dir, "iAT2_pval_MF.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.padj.CC, paste0(Fibro.dir, "iAT2_padj_CC.csv"), 
          quote = F, row.names = F)

AT2.padj.CC <- cbind(Overlap = AT2.sig.CC$Term %in% AT2.padj.overlapCC, 
                     AT2.sig.CC)
head(AT2.padj.CC[, 1 : 5])
write.csv(AT2.padj.CC, paste0(Fibro.dir, "AT2_padj_CC.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.pval.CC, paste0(Fibro.dir, "iAT2_pval_CC.csv"), 
          quote = F, row.names = F)
AT2.pval.CC <- cbind(Overlap = AT2.sig.CC$Term %in% AT2.overlapped.CC, 
                      AT2.sig.CC)
head(AT2.pval.CC[, 1 : 5])
write.csv(AT2.pval.CC, paste0(Fibro.dir, "AT2_pval_CC.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.padj.BP, paste0(Fibro.dir, "iAT2_padj_BP.csv"), 
          quote = F, row.names = F)

AT2.padj.BP <- cbind(Overlap = AT2.sig.BP$Term %in% AT2.padj.overlapBP, 
                     AT2.sig.BP)
head(AT2.padj.BP[, 1 : 5])
write.csv(AT2.padj.BP, paste0(Fibro.dir, "AT2_padj_BP.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.pval.BP, paste0(Fibro.dir, "iAT2_pval_BP.csv"), 
          quote = F, row.names = F)

AT2.pval.BP <- cbind(Overlap = AT2.sig.BP$Term %in% AT2.pval.overlapBP, 
                     AT2.sig.BP)
head(AT2.pval.BP[, 1 : 5])
write.csv(AT2.pval.BP, paste0(Fibro.dir, "AT2_pval_BP.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.padj.KEGG, paste0(Fibro.dir, "iAT2_padj_KEGG.csv"), 
          quote = F, row.names = F)

AT2.padj.KEGG <- cbind(Overlap = AT2.sig.KEGG$Term %in% AT2.padj.overlapKEGG, 
                     AT2.sig.KEGG)
head(AT2.padj.KEGG[, 1 : 5])
write.csv(AT2.padj.KEGG, paste0(Fibro.dir, "AT2_padj_KEGG.csv"), 
          quote = F, row.names = F)
...
write.csv(iAT2.pval.KEGG, paste0(Fibro.dir, "iAT2_pval_KEGG.csv"), 
          quote = F, row.names = F)

AT2.pval.KEGG <- cbind(Overlap = AT2.sig.KEGG$Term %in% AT2.pval.overlapKEGG, 
                       AT2.sig.KEGG)
head(AT2.pval.KEGG[, 1 : 5])
write.csv(AT2.pval.KEGG, paste0(Fibro.dir, "AT2_pval_KEGG.csv"), 
          quote = F, row.names = F)
